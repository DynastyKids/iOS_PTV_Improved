...
//  Oh_My-Transport
//
//  Created by OriWuKids on 26/5/19.
//  Copyright © 2019 wgon0001. All rights reserved.
//

// Another method of doing Directions View Controller leave as seperate.
...
struct finalStop{
    var finalStopId: Int?
    var finalStopName: String?
    var directionId: Int?
    init(finalStopId: Int?, finalStopName: String?, directionId: Int?) {
...
class Directions2ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    var disruptiondata: [Disruption] = []   // Disruption information for this route
    var allstopsdata: [stopOnRoute] = []    // All stops data for this route
    var sortedStops: [stopOnRoute] = []     // All stops data for this route, sorted by user current location
    var runs: [Run] = []                    // Running datas (include, runId, routeId, routeType, destinationName, directionId, vehiclePosition)
    
    var destinations: Int = 0                  // For row count, store different destinations
    var finalStops: [finalStop] = []
    ...
    override func viewDidLoad() {
        ...
        _ = URLSession.shared.dataTask(with: URL(string: showRouteInfo(routeId: routeId))!){ (data, response, error) in
            ...
            do {
                ...
                if routeData.route?.routeNumber != nil{
                    ...
                    self.routeName = routeData.route?.routeName ?? ""   //改用GTFS_id
                }
        ...
        _ = URLSession.shared.dataTask(with: URL(string: showRoutesStop(routeId: routeId, routeType: routeType))!){(data, response, error) in
            ...
            do{
                ...
                guard stopsdata.stops != nil else {
                    return
                }
        ...
        _ = URLSession.shared.dataTask(with: URL(string: showRouteRuns(routeId: routeId))!){ (data, response, error) in
            ...
            do{
                ...
                guard runsdata.runs != nil else{
                    print("Empty on Running data")
                    return
                }
                self.runs = runsdata.runs!  //有n个终点方向，有routeId，RouteType,directionId
                // 所有车站数据存在了allstopsdata
                
                for each in self.runs{
                    if self.finalStops.count == 0{
                        self.finalStops.append(finalStop.init(finalStopId: each.finalStopId, finalStopName: each.destinationName, directionId: each.directionId))
                    }else{  // 检查有没有这个终点站的地址
                        var flag = true
                        for eachDestination in self.finalStops {
                            if eachDestination.finalStopId == each.finalStopId{
                                flag = false
                            }
                        }
                        if flag == true{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1{
            ...
            let currentDestination = finalStops[indexPath.row]
            cell.directionNameLabel.text = currentDestination.finalStopName
            // 每个终点方向的数据由Cell自身去Fetch
            var filteredRuns: [Run] = []
            for each in runs{
                if each.finalStopId == finalStops[indexPath.row].finalStopId{
                    filteredRuns.append(each)
                }
            }
